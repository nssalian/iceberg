...
#
# Usage:
#   ./benchmark/scripts/run_spark.sh          # all scales
#   ./benchmark/scripts/run_spark.sh small    # just small
#   ./benchmark/scripts/run_spark.sh medium   # just medium
#   ./benchmark/scripts/run_spark.sh large    # just large
...
WRITE_OPS="write-json,write-variant,write-shredded,write-shredded-buf10,write-shredded-buf1000,write-shredded-buf10000"
READ_OPS="read-baseline,read-project-json,read-project-variant,read-project-shredded,read-project-shredded-buf10,read-project-shredded-buf1000,read-project-shredded-buf10000,read-nested-json,read-nested-variant,read-nested-shredded,read-filter-json,read-filter-variant,read-filter-shredded,read-agg-json,read-agg-variant,read-agg-shredded"
...
run_spark() {
  ...
  local write_results="${BENCH_DIR}/runs/$(date +%Y%m%d_%H%M%S)_spark_writes_${scale}"
  ...
  local read_results="${BENCH_DIR}/runs/$(date +%Y%m%d_%H%M%S)_spark_reads_${scale}"
...
echo "Results:"
ls -dt "${BENCH_DIR}"/runs/*spark* 2>/dev/null | head -20
